...
const savePermissions = () => {
  ...
  form.post(route('accessibility.role-permissions', selectedRole.value.id), {
    forceFormData: true,
    preserveScroll: true,
    preserveState: true,
    onSuccess: (response: any) => {
       router.visit(route('accessibility.index'),{
            preserveState: true,
            only: ['roles', 'permissions', 'groupedPermissions', 'assignedPermissions'],
        });
      toast.success(`Successfully updated permissions for '${selectedRole.value?.name}'.`);
    },
        ...
        <div v-if="selectedRole" class="grid gap-8">
          ...
          <div class="sticky bottom-4 flex justify-end">
            <Button @click.prevent="savePermissions" :disabled="isSubmitting" variant="default"
              class="px-6 py-3 rounded cursor-pointer  transition">
              <!-- 💾 Save Changes -->
              Save Changes
            </Button>
          </div>
